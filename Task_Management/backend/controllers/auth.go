...
func Register(c *gin.Context) {
	...
	query := "INSERT INTO admins (admin_id, username, email, password_hash) VALUES (?, ?, ?, ?)"
...
func isUserRegistered(username string) bool {
	...
	if err := utils.Session.Query("SELECT admin_id FROM admins WHERE username=? ALLOW FILTERING", username).Scan(&existingUser.Admin_id); err != nil {
...
func authenticateUser(username, password string) (gocql.UUID, error) {
	...
	if err := utils.Session.Query("SELECT admin_id FROM admins WHERE username=? AND password_hash=? ALLOW FILTERING", username, password).Scan(&userID); err != nil {
